...
class Laporan extends BaseController
{
	...
	public function historiBayar()
	{
		...
		$where = [
			'siswa.nisn' => $this->request->getPost('txtNoIndukSiswaNasional'),
		];

		$data['listPembayaran'] = $this->bayar->where($where)->orderBy('tgl_bayar', 'asc')->findAll();
		...
		$htmlData .= '
					<tr class="bg-light">
					<td colspan="4" class="text-center font-weight-bold">Total Pembayaran</td>
	...
	public function tampilkanPenerimaan()
	{
		...
		foreach ($data['listPembayaran'] as $row) {
			...
			$htmlData .= '<td>' . $no . '</td>';
			$htmlData .= '<td>' . $row['tahun_dibayar'] . '</td>';
			$htmlData .= '<td>' . $arrBulan[$row['bulan_dibayar']] . '</td>';
			$htmlData .= '<td>' . $row['tgl_bayar'] . '</td>';
			$htmlData .= '<td class="text-right">' . number_format($row['jumlah_bayar'], 0, ',', '.') . '</td>';
			$htmlData .= '</tr>';
		}
