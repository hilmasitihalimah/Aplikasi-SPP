...
class Kelas extends BaseController
{
	...
	public function simpanKelas()
	{
		...
		$kelas = $this->kelas->where('nama_kelas', $data['nama_kelas'])->findAll();
		if (count($kelas) == 1) {
			return redirect()->to('/kelas')->with('pesan-error', '<div class="alert alert-danger">Nama kelas sudah ada !</div>');
		} else {
		}
